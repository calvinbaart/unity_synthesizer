...
public class ComboBox
{
                                    ...
                                    GUIStyle buttonStyle, GUIStyle boxStyle, GUIStyle listStyle)
    {
        ...
        if (isClickedComboButton)
        {
            Rect listRect = new Rect(rect.x, rect.y + (rect.height / 2.0f) + listStyle.CalcHeight(listContent[0], 1.0f),
                      rect.width, listStyle.CalcHeight(listContent[0], 1.0f) * listContent.Length);

            Rect viewRect = new Rect(0, 0, listRect.width * 0.95f, rect.height * 5);

            GUILayout.BeginArea(new Rect(listRect.x, listRect.y, listRect.width, viewRect.height));
            GUILayout.Box("", boxStyle, GUILayout.Width(listRect.width), GUILayout.Height(viewRect.height));
            GUILayout.EndArea();
